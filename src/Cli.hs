...
runCommand = \case
  ...
  StartAgent config -> do
    Logger.infoM "quad.server" "Agent starting..."
  ...
  Butcher.addCmd "start-agent" do
    ...
    endpoint <-
      Butcher.addParamString "ENDPOINT" $
        Butcher.paramHelpStr "Server endpoint" <> Butcher.paramSuggestions [defaultEndpoint] <> Butcher.paramDefault defaultEndpoint
